...
class GeocodersHandler extends jeo_media_partners_UnitTestCase {


	function test_register() {

		jeo_media_partners_geocode_handler()->register_geocoder([
			'slug' => 'test',
			'name' => 'Test',
		...
		$slugs = array_map(function($e) {return $e['slug']; }, jeo_media_partners_geocode_handler()->get_registered_geocoders());

		$this->assertContains('test', $slugs);

		$object = jeo_media_partners_geocode_handler()->initialize_geocoder('test');
	...
	function test_get_active() {
		...
		$active = jeo_media_partners_geocode_handler()->get_active_geocoder();
	...
	function test_disable_update_of_protected_meta() {
		...
		foreach ( \jeo_media_partners_geocode_handler()->geo_attributes as $key ) {
